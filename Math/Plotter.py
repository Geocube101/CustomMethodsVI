...
import CustomMethodsVI.Math.Statistics.Functions as Stats
...
class CartesianScatterPlot2D(AxisPlot2D, MultiPlot2D[Plot2D[tuple[float, float]]]):
	...
	def polynomial_regression(self, degree: int = None, max_degree: int = None, fit_method='poly', *, plot_name: typing.Optional[str] = ...):
		...
		return (f'y={equation}; r²={r2}', r2, lambda x_, f=factors: sum(m * x ** (len(f) - b - 1) for b, m in enumerate(f))) if len(factors) > 0 else (None, None, lambda x_: None)
