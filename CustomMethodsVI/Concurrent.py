...
class Promise:
	...
	def throw(self, err: BaseException) -> None:
		...
		if self.__response__ is not None:
			...
			Misc.raise_ifn(isinstance(err, BaseException), Exceptions.InvalidArgumentException(Promise.throw, 'err', type(err)))
...
class ConcurrentPromise(Promise):
	...
	def throw(self, err: BaseException) -> None:
		...
		if os.getpid() == self.__src__:
			...
			Misc.raise_ifn(isinstance(err, BaseException), Exceptions.InvalidArgumentException(ConcurrentPromise.throw, 'err', type(err)))
...
class ThreadedPromise(Promise):
	...
	def throw(self, err: BaseException) -> None:
		...
		if threading.current_thread().native_id == self.__src__:
			...
			Misc.raise_ifn(isinstance(err, BaseException), Exceptions.InvalidArgumentException(ThreadedPromise.throw, 'err', type(err)))
			self.__response__ = (False, err)
